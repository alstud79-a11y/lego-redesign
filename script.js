...
function getYouTubeEmbedUrl(value) {
  ...
  try {
    ...
    return id ? `https://www.youtube.com/embed/${id}?autoplay=1&rel=0&controls=0&playsinline=1&fs=0` : '';
  } catch (_) {
    return '';
  }
}
...
function applyLandmarkToModal(landmarkKey) {
  ...
  const collectionLocation = document.querySelector('#buildModalCollectionLocation');
  const landmarkKeys = Object.keys(LANDMARK_DATA);
  if (collectionIndex) collectionIndex.textContent =
    `${String(landmarkKeys.indexOf(currentLandmarkKey) + 1).padStart(2, '0')} / ${String(landmarkKeys.length).padStart(2, '0')}`;
  if (collectionLocation) collectionLocation.textContent =
    `${data.cityMark} · ${data.country.split(' · ')[0]}`;
